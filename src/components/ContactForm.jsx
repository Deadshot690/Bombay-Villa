...
function ContactForm() {
  ...
  const handleSubmit = (e) => {
    ...
    if (!firstName || !lastName || !phone) {
      ...
      const msg = `Hello, my name is ${values.firstName} ${values.lastName}.\nPhone: ${values.phone}${values.email ? `\nEmail: ${values.email}` : ''}${values.message ? `\nMessage: ${values.message}` : ''}`;
      const whatsappUrl = `https://wa.me/919730877806?text=${encodeURIComponent(msg)}`;
      window.open(whatsappUrl, "_blank");
      setValues({ ...initialState, submit: "Redirecting to WhatsApp..." });
      setTimeout(() => {
